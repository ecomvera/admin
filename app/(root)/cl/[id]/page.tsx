...
const Page = ({ params }: { params: { id: string } }) => {
  ...
  const products = useSWR(`/api/products?group-data`, fetcher, {
    ...fetchOpt,
    revalidateOnMount: true,
  });
  ...
  const [values, setValues] = React.useState<{ name?: string; isActive?: boolean; image?: string; banner?: string }>();
  ...
  const handleImage = async (e: ChangeEvent<HTMLInputElement>, type: string) => {
    ...
    if (e.target.files) {
      ...
      const publicId = getPublicId(data?.image);
  ...
  if (category.isLoading) return <div className="text-center text-xl text-light-3">Loading...</div>;
  if (!category.data?.data) return <div className="text-center text-xl text-light-3">No data</div>;
